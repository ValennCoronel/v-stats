...
export default async function SharedTeamStatsPage({
  ...
  if (playerId) {
    const playerStats = await getPlayerDashboardStats(shareLink.clubId, playerId, shareLink.teamId)
    if (!playerStats) {
      notFound()
    }

    const totalSets = playerStats.setsWon + playerStats.setsLost
    ...
    return (
      ...
      >
        <div style={{ maxWidth: 1100, margin: "0 auto", padding: "32px 20px 64px" }}>
            ...
            <div style={{ display: "flex", justifyContent: "space-between", gap: 24, flexWrap: "wrap" }}>
              <div>
                ...
                <p style={{ margin: 0, fontSize: 16, opacity: 0.8 }}>
                  {shareLink.team.name} - {getPositionLabel(playerStats.player.position)}
                </p>
              </div>
              <div
                style={{
              ...
              >
                {[
                  { label: "Partidos", value: playerStats.totalMatches },
                  { label: "Puntos", value: playerStats.totals.puntos },
                  { label: "Eficiencia", value: `${playerStats.efficiency}%` },
                  { label: "Record", value: `${playerStats.wins}-${playerStats.losses}` },
                ].map((item) => (
                  <div
                    key={item.label}
                    style={{
          ...
          >
            {[
              { label: "Sets", value: `${playerStats.setsWon}-${playerStats.setsLost}`, detail: totalSets > 0 ? `${Math.round((playerStats.setsWon / totalSets) * 100)}% ganados` : "Sin sets cargados" },
              { label: "Positivas", value: playerStats.positiveActions, detail: `${playerStats.actionsPerMatch} acciones por partido` },
              { label: "Errores", value: playerStats.negativeActions, detail: `${playerStats.errorsPerMatch} por partido` },
              { label: "Promedio", value: playerStats.pointsPerMatch, detail: "puntos por partido" },
            ].map((card) => (
              <article
                key={card.label}
                style={{
          ...
          <section style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(320px, 1fr))", gap: 20, marginTop: 24 }}>
            ...
            >
              <div style={{ fontSize: 12, letterSpacing: 1.3, color: "#64748b", textTransform: "uppercase" }}>Desglose</div>
              <h2 style={{ margin: "10px 0 0", fontSize: 28 }}>Acciones registradas</h2>

              <div style={{ marginTop: 18, display: "grid", gap: 12 }}>
                {[
                  { label: "Puntos", value: playerStats.totals.puntos, color: "#1E6FD9" },
                  { label: "Ataques", value: playerStats.totals.ataques, color: "#0EA5E9" },
                  { label: "Bloqueos", value: playerStats.totals.bloqueos, color: "#7C3AED" },
                  { label: "Aces", value: playerStats.totals.aces, color: "#F59E0B" },
                  { label: "Defensas", value: playerStats.totals.defensas, color: "#16A34A" },
                  { label: "Errores", value: playerStats.negativeActions, color: "#DC2626" },
                ].map((item) => (
                  <div
                    key={item.label}
                    style={{
            ...
            >
              ...
              <div style={{ marginTop: 18, display: "grid", gap: 12 }}>
                {playerStats.matchTimeline.length > 0 ? playerStats.matchTimeline.map((match) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <div style={{ marginTop: 14, display: "flex", justifyContent: "space-between", alignItems: "end", gap: 12 }}>
                        <div style={{ fontSize: 20, fontWeight: 800 }}>{match.finalScore || "-"}</div>
                        <div style={{ textAlign: "right", color: getEfficiencyColor(match.eficiencia), fontSize: 13, fontWeight: 700 }}>
                          {match.puntos} pts - {match.eficiencia}% eficiencia
                        </div>
                      </div>
                    </div>
                  )
                  ...
                  <div style={{ padding: "28px 4px", color: "#64748b" }}>
                    Este jugador todavia no tiene partidos finalizados para mostrar.
                  </div>
                )}
              </div>
            </article>
